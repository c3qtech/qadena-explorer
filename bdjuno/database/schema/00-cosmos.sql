...
/**
 * This function is used to find all the message types that involve any of the given addresses.
 * It returns messages that can be joined with message_type table to get type information.
 */
CREATE FUNCTION messages_types_by_address(
    addresses TEXT[],
    "limit" BIGINT = 100,
    "offset" BIGINT = 0)
    RETURNS SETOF message AS
$$
SELECT DISTINCT ON (type) * FROM message
